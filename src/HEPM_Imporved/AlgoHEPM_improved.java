 ...
 */
public class AlgoHEPM_improved {
     ...
     */
    public Itemsets runAlgorithm(double minEfficiency, String inputPath, String outputPath,Stock stock, boolean activateTransactionMerging, int maximumTransactionCount, boolean activateSubtreeUtilityPruning) throws IOException {
       ...
       for (int j=0; j< itemsToKeep.size(); j++)
	   {
		...
		if(activateSubtreeUtilityPruning){
			...
			for(Integer item : itemsToKeep){
				double estimateEfficiency=(double)utilityBinArraySU[item]/stock.investMap.get(newNamesToOldNames[item]); 		
		   		if (Tool.compare(estimateEfficiency, minEfficiency) >= 0) {
...
//		        }
	    	...
	    	for (int k = j+1; k < itemsToKeep.size(); k++) {
	        	...
	        	double efficiencySU = (double) utilityBinArraySU[itemk]/(InvestPE+stock.investMap.get(newNamesToOldNames[itemk]));
	        	double efficiencyLU = (double) utilityBinArrayLU[itemk]/(InvestPE+stock.investMap.get(newNamesToOldNames[itemk]));
	        	// if the sub-tree utility is no less than min util
	        	//System.out.println(efficiencyLU);
...
//				}
//			}
//			System.out.println("itemset: "+buffer);
//			String result = "";
//			for (Integer integer : newItemsToExplore) {
//				result += " " + integer;
//			}
//			System.out.println(result);
//			result = "";
//			for (Integer integer : newItemsToKeep) {
//				result += " " + integer;
//			}
//			System.out.println(result);
...
//			writer.newLine();
	...
	public void printStats() {
		...
		System.out.println("========== HEPM_Imporved - STATS ============");
