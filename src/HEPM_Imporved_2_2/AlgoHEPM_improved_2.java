 ...
 *         Souza
 */
public class AlgoHEPM_improved_2 {
	 ...
	 */
	public Itemsets runAlgorithm(double minEfficiency, String inputPath, String outputPath, Stock stock,
		...
		/*
		 * // if the user choose to save to file // create object for writing the output
		 * file if(outputPath != null) { writer = new BufferedWriter(new
		 * FileWriter(outputPath)); }else { // if the user choose to save to memory
		 * writer = null; this.highUtilityItemsets = new Itemsets("Itemsets"); }
		 */
...
//    	//======
	...
	private void backtrackingEFIM(List<Transaction> transactionsOfP, List<Integer> itemsToKeep,
			List<Integer> itemsToExplore, int SumOfInvest, int prefixLength) throws IOException {
...
//		        }
			...
			if (Tool.compare(efficiency, minEfficiency) >= 0) {
				output(prefixLength, efficiency, utilityPe, InvestPE);
			}

			// ==== Next, we will calculate the Local Utility and Sub-tree utility of
			// all items that could be appended to PU{e} ====
			useUtilityBinArraysToCalculateUpperBounds(transactionsPe, j, itemsToKeep, InvestPE);
			...
			if (activateSubtreeUtilityPruning) {
				...
				// items
				backtrackingEFIM(transactionsPe, newItemsToKeep, newItemsToExplore, InvestPE, prefixLength + 1);
			} else {
				// if sub-tree utility pruning is deactivated, we consider secondary items also
				// as primary items
				backtrackingEFIM(transactionsPe, newItemsToKeep, newItemsToKeep, InvestPE, prefixLength + 1);
			}
		}
	...
	private void useUtilityBinArraysToCalculateUpperBounds(List<Transaction> transactionsPe, int j,
			List<Integer> itemsToKeep, int SumInvest) {
		...
		for (Transaction transaction : transactionsPe) {
			...
			for (int i = transaction.getItems().length - 1; i >= transaction.offset; i--) {
				...
				if (contains) {
					...
					efficiencyBinArraySU[item] += sumRemainingUtility * 1.0d / (SumInvest +stock.investMap.get(newNamesToOldNames[item]))
							+ (transaction.prefixUtility * 1.0d + utility) / (SumInvest+stock.investMap.get(newNamesToOldNames[item]));
					...
					efficiencyBinArrayLU[item] += (transaction.prefixUtility * 1.0d + utility)
							/ (SumInvest +stock.investMap.get(newNamesToOldNames[item]))
	 ...
	 */
	private void output(int tempPosition, double efficiency, int Util, int Invest) throws IOException {
		patternCount++;
...
//			writer.newLine();
	...
	public void printStats() {
		...
		System.out.println("========== HEPM_Imporved_2 - STATS ============");
